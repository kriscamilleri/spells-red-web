      ...
      <div class="header mb-6" v-for="(h, i) in subClassHeaders" :key="i">
        ...
        >
          <input
            ...
            class="mr-1 p-1"
          />
          <label :for="`subClassCheckbox_${h}_${i}`" class="p-1">{{
            c.subClass
          }}</label>
        </div>
        <br />
      ...
      <div class="header mb-6" v-for="(h, i) in variantClassHeaders" :key="i">
        <h3 class="text-lg line-behind flex flex-col">
          <div class="flex flex-row justify-between tweak-width max-w-[80%]">
            <span>
              ...
              />
              <label :for="`variantClassSelectAll_${h}`" class="p-1">{{
                h
              }}</label>
            </span>
          </div>
        ...
        >
          <input
            ...
            class="mr-1 p-1"
          />
          <label :for="`variantClassCheckbox_${h}_${i}`" class="p-1">{{
            c.variantSource
          }}</label>
        </div>
        <br />
...
export default {
  ...
  computed: {
    ...
    subClassHeaders() {
      ...
      const sortedHeaders = classHeadersWithCount
  ...
  methods: {
    ...
    selectSubClassOfBaseClass(baseClass) {
      let allRelatedSubclasses = this.subClasses.filter(
        (c) => c.baseClass === baseClass
      );
      let allChosenRelatedSubClasses = this.selectedSubClasses.filter(
        (c) => c.baseClass === baseClass
      );
      const areAllSelected =
        allRelatedSubclasses.length === allChosenRelatedSubClasses.length;
      const areNoneSelected = allChosenRelatedSubClasses.length === 0;
      if (areAllSelected) {
        //if yes, remove all with matching base class from selected list
        this.selectedSubClasses = this.selectedSubClasses.filter(
          (c) => c.baseClass !== baseClass
        );
        ...
        allRelatedSubclasses.forEach((c) => {
          this.selectedSubClasses.push(c);
        });
    ...
    selectVariantClassOfBaseClass(baseClass) {
      let allRelatedVariantClasses = this.variantClasses.filter(
        (c) => c.baseClass === baseClass
      );
      let allChosenVariantSubClasses = this.selectedVariantClasses.filter(
        (c) => c.baseClass === baseClass
      );
      const areAllSelected =
        allRelatedVariantClasses.length === allChosenVariantSubClasses.length;
      const areNoneSelected = allChosenVariantSubClasses.length === 0;
      if (areAllSelected) {
        //if yes, remove all with matching base class from selected list
        this.selectedVariantClasses = this.selectedVariantClasses.filter(
          (c) => c.baseClass !== baseClass
        );
        this.selectedVariantClassHeaderState[baseClass] = false;
      } else if (areNoneSelected) {
        //if no, add all with matching base class to selected list
        allRelatedVariantClasses.forEach((c) => {
          this.selectedVariantClasses.push(c);
        });
  ...
  watch: {
    selectedSubClasses(selectedSubClasses) {
      const baseClasses = [
        ...new Set(selectedSubClasses.map((c) => c.baseClass)),
      ];
      for (let i = 0; i < baseClasses.length; i++) {
        let selectedCount = selectedSubClasses.filter(
          (c) => c.baseClass === baseClasses[i]
        ).length;
        let totalCount = this.subClasses.filter(
          (c) => c.baseClass === baseClasses[i]
        ).length;
        this.selectedSubClassHeaderState[baseClasses[i]] =
    ...
    selectedVariantClasses(selectedVariantClasses) {
      const baseClasses = [
        ...new Set(selectedVariantClasses.map((c) => c.baseClass)),
      ];
      for (let i = 0; i < baseClasses.length; i++) {
        let selectedCount = selectedVariantClasses.filter(
          (c) => c.baseClass === baseClasses[i]
        ).length;
        let totalCount = this.variantClasses.filter(
          (c) => c.baseClass === baseClasses[i]
        ).length;
        this.selectedVariantClassHeaderState[baseClasses[i]] =
          selectedCount === totalCount;
      }
    },
    subClassHeaders() {
